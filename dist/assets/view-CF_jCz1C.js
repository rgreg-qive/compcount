import{C as x,T as f}from"./themeManager-CRIQpY6X.js";class y{constructor(){this.chartManager=new x,this.themeManager=new f,this.init()}async init(){var e,t,o,n;try{console.log("🚀 Iniciando ViewApp..."),this.setupThemeToggle();const a=new URLSearchParams(window.location.search);if(console.log("🔍 Parâmetros da URL:",Object.fromEntries(a.entries())),!(a.get("shared")==="true")){console.log("❌ Não é um link compartilhado válido"),this.showError("Este link não é uma análise compartilhada válida.");return}console.log("✅ Link compartilhado válido detectado");const r=this.extractAnalysisFromUrl(a);if(!r){console.log("❌ Nenhum dado de análise encontrado"),this.showError("Dados da análise não encontrados no link.");return}console.log("✅ Dados da análise carregados com sucesso!"),console.log("📊 Resumo dos dados:",{frameInfo:((e=r.frameInfo)==null?void 0:e.name)||"N/A",totalComponents:((t=r.components)==null?void 0:t.length)||0,connected:((o=r.summary)==null?void 0:o.connected)||0,disconnected:((n=r.summary)==null?void 0:n.disconnected)||0}),this.displayAnalysis(r)}catch(a){console.error("❌ Erro ao inicializar visualização:",a),this.showError("Erro ao carregar análise compartilhada.")}}setupThemeToggle(){const e=document.getElementById("theme-toggle"),t=document.getElementById("theme-icon");e&&t&&(t.textContent=this.themeManager.getThemeIcon(),e.addEventListener("click",()=>{this.themeManager.toggleTheme(),t.textContent=this.themeManager.getThemeIcon(),console.log(`🎨 Tema alterado para: ${this.themeManager.getThemeName()}`)}),window.addEventListener("themeChanged",o=>{t.textContent=this.themeManager.getThemeIcon(),console.log(`🎨 Tema alterado: ${o.detail.isDark?"dark":"light"}`)}))}extractAnalysisFromUrl(e){var t,o;try{const n=e.get("connected"),a=e.get("disconnected"),s=e.get("compliance"),r=e.get("status"),d=e.get("timestamp");if(!n||!a||!s||!r)return console.log("❌ Parâmetros básicos da URL não encontrados"),null;const l=this.tryLoadFullAnalysisData(e.get("id"));if(l)return console.log("✅ Dados completos carregados do storage!"),l;const i=e.get("data");if(i){console.log("🔍 Tentando decodificar dados da URL...");try{const g=atob(decodeURIComponent(i)),c=JSON.parse(g);if(console.log("✅ Dados decodificados da URL com sucesso!"),console.log("📊 Componentes encontrados:",((t=c.components)==null?void 0:t.length)||0),console.log("📊 Componentes excluídos:",((o=c.excludedComponents)==null?void 0:o.length)||0),c&&e.get("id")){const u=`shared-analysis-${e.get("id")}`;try{localStorage.setItem(u,JSON.stringify(c)),console.log("💾 Dados salvos no localStorage para uso futuro")}catch(h){console.warn("⚠️ Não foi possível salvar no localStorage:",h)}}return c}catch(g){console.error("❌ Erro ao decodificar dados da URL:",g)}}console.log("⚠️ Usando apenas dados básicos da URL");const m=parseInt(n),p=parseInt(a);return{frameInfo:{name:"Análise Compartilhada",nodeId:"shared",url:"#"},summary:{connected:m,disconnected:p,total:m+p},components:[],timestamp:d?parseInt(d):Date.now(),complianceRate:parseFloat(s.replace("%","")),complianceStatus:decodeURIComponent(r),analysisId:e.get("id")||void 0}}catch(n){return console.error("❌ Erro ao extrair dados da URL:",n),null}}tryLoadFullAnalysisData(e){var t,o,n;if(!e)return console.log("🔍 Nenhum ID fornecido para buscar dados completos"),null;try{const a=`shared-analysis-${e}`;console.log("🔍 Tentando carregar dados do localStorage com chave:",a);const s=localStorage.getItem(a);if(s){console.log("✅ Dados encontrados no localStorage!");const d=JSON.parse(s);return console.log("📊 Dados carregados:",{components:((t=d.components)==null?void 0:t.length)||0,excludedComponents:((o=d.excludedComponents)==null?void 0:o.length)||0,frameInfo:((n=d.frameInfo)==null?void 0:n.name)||"N/A"}),d}else console.log("❌ Nenhum dado encontrado no localStorage");const r=sessionStorage.getItem(a);return r?(console.log("✅ Dados encontrados no sessionStorage!"),JSON.parse(r)):(console.log("❌ Nenhum dado encontrado no sessionStorage"),console.log("🔍 Chaves disponíveis no localStorage:",Object.keys(localStorage)),null)}catch(a){return console.error("❌ Erro ao carregar dados completos:",a),null}}displayAnalysis(e){const t=document.getElementById("loading-state");t&&t.classList.add("hidden");const o=document.getElementById("analysis-content");o&&o.classList.remove("hidden"),this.updateBasicInfo(e),this.updateChart(e),this.updateSummary(e),this.updateComponentsTable(e)}updateBasicInfo(e){const t=document.getElementById("frame-name");t&&(t.textContent=e.frameInfo.name);const o=document.getElementById("analysis-date");if(o){const s=new Date(e.timestamp);o.textContent=`Analisado em ${s.toLocaleDateString("pt-BR")} às ${s.toLocaleTimeString("pt-BR")}`}const n=document.getElementById("figma-link");n&&e.frameInfo.url!=="#"?n.href=e.frameInfo.url:n&&(n.style.display="none");const a=document.getElementById("compliance-status");a&&(a.textContent=e.complianceStatus,a.className=`px-2 py-1 text-xs font-medium rounded-full ${e.complianceRate>=80?"bg-green-100 text-green-800":"bg-red-100 text-red-800"}`)}updateChart(e){this.chartManager.updatePieChartWithValues(e.summary.connected,e.summary.disconnected)}updateSummary(e){const t=document.getElementById("summary-cards");if(!t)return;const o=e.summary.total>0?Math.round(e.summary.connected/e.summary.total*100):0;t.innerHTML=`
    ...
    `}createFigmaElementUrl(e,t){try{if(e==="#"||!e)return null;const o=new URL(e),n=t.replace(":","-");return o.searchParams.set("node-id",n),o.toString()}catch(o){return console.error("Erro ao criar URL do elemento Figma:",o),null}}updateComponentsTable(e){const t=document.getElementById("components-table-body");if(!t)return;t.innerHTML="";const o=this.getAllComponents(e);if(o.length===0){const n=document.createElement("tr");n.innerHTML=`
      ...
      `,t.appendChild(n);return}o.forEach(n=>{const{component:a,isIncluded:s}=n,r=document.createElement("tr");r.className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700";const d=a.isConnectedToDS?'<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200">Conectado</span>':'<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200">Desconectado</span>',l=this.createFigmaElementUrl(e.frameInfo.url,a.nodeId),i=l?`<a href="${l}" target="_blank" class="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 underline font-mono text-sm" title="Abrir elemento no Figma">${a.nodeId} 🔗</a>`:`<span class="font-mono text-sm text-gray-500 dark:text-gray-400">${a.nodeId}</span>`,m=s?"Sim":"Não";r.innerHTML=`
      ...
      `,t.appendChild(r)})}getAllComponents(e){const t=[],o=new Set;try{const n=localStorage.getItem(`shared-analysis-${e.analysisId||"unknown"}`);if(n){const a=JSON.parse(n);if(a.components&&Array.isArray(a.components)&&a.components.forEach(s=>{o.has(s.nodeId)||(t.push({component:s,isIncluded:!0}),o.add(s.nodeId))}),a.excludedComponents&&Array.isArray(a.excludedComponents)&&a.excludedComponents.forEach(s=>{o.has(s.nodeId)||(t.push({component:s,isIncluded:!1}),o.add(s.nodeId))}),t.length>0)return console.log(`✅ Carregados ${t.length} componentes únicos do localStorage`),t}}catch(n){console.warn("Não foi possível recuperar dados completos do localStorage:",n)}return e.components.forEach(n=>{o.has(n.nodeId)||(t.push({component:n,isIncluded:!0}),o.add(n.nodeId))}),console.log(`✅ Carregados ${t.length} componentes únicos da URL`),t}showError(e){const t=document.getElementById("loading-state");t&&t.classList.add("hidden");const o=document.getElementById("error-state");o&&o.classList.remove("hidden");const n=document.getElementById("error-message");n&&(n.textContent=e)}}document.addEventListener("DOMContentLoaded",()=>{new y});
